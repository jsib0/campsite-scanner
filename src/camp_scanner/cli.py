...
from camp_scanner.campgrounds import CAMPGROUNDS
from camp_scanner.config import (
    DEFAULT_LOG_FILE,
    NtfyConfig,
    SmsConfig,
    load_env_file,
...
from camp_scanner.logging_setup import LOGGER, configure_logging
from camp_scanner.messengers import send_message
from camp_scanner.monitor import monitor_campground
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=40.0,
        help="Seconds between checks of each campground (default: 40).",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
        help=(
            "Load settings from this file. Otherwise CAMP_SCANNER_ENV_FILE is "
            "used when set."
        ),
    )
    parser.add_argument(
